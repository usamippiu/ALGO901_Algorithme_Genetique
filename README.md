...
## 2. Benchmarks
Avec les paramètres _ranges_=[-500, 500]^2, _taille\_population_=100, _nb\_iter\_max_=500, codage=```CodageBinaire([52, 10])```, selection=```RoueDeLaFortune()```, croisement=```CroisementDouble()``` et mutation=```MutationParPermutation(0.1)```.
