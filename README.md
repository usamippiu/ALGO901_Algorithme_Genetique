...
## 2. Benchmarks
Avec les paramètres _ranges_=[-500, 500]^2, taille_population=100, nb_iter_max=500, codage=```CodageBinaire([52, 10])```, selection=```RoueDeLaFortune()```, croisement=```CroisementDouble()``` et mutation=```MutationParPermutation(0.1)```.
